function [imSkeleton minutiaArray] = cleanskeleton(imOriginal, imSegmented, imContour, imSkeleton, orientationArray)

%[imSkeleton minutiaArray] = cleanskeleton(imOriginal, imSegmented, imContour, imSkeleton, orientationArray)
%
% Funkce odstranuje ze skeletizovaneho obrazku otisku nechtene artefakty
% jako zebriky, vyrustky a preruseni linii pomoci aplikace pravidel.
%
%       imOriginal - vstupni otisk
%       imContour - segmentace otisku (imContour vystup z funkce segmentimage)
%       imSkeleton - skeletonizovany otisk
%       orientationArray - orientacni pole


warning off all

Cs.skeleton = imSkeleton;
Cs.contour = imContour;
Cs.orientationArray = imresize(orientationArray, size(imOriginal));
...
imSkeleton = Cs.skeleton;
width = size(imSkeleton, 2);
height = size(imSkeleton, 1);
imSkeleton = imSkeleton(26:height-25,26: width-25);
minutiaArray = Cs.minutiaArray;         %matrix of minutiae image is stored in Fp.minutiaArray
